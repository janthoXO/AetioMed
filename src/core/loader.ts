...
function topoSort(extensions: AnyExt[]): AnyExt[] {
  ...
  function visit(ext: AnyExt) {
    ...
    if (visiting.has(ext.name)) throw new Error(`Circular dependency: ${ext.name}`)
    visiting.add(ext.name)
    for (const dep of ext.dependsOn) {
      if (!byName.has(dep.name))
        throw new Error(`"${ext.name}" depends on unregistered extension "${dep.name}"`)
...
export async function loadExtensions(opts: LoaderOptions): Promise<void> {
  ...
  for (const ext of sorted) {
    ...
    if (missingFlags.length > 0) {
      console.log(`[loader] ⏭  "${ext.name}" — missing flags: [${missingFlags.join(', ')}]`)
    ...
    if (skippedDep) {
      console.log(`[loader] ⏭  "${ext.name}" — dep "${skippedDep.name}" was skipped`)
      skipped.add(ext.name)
      continue
    }
    ...
    if (ext.envSchema) {
      ...
      if (!result.success) {
        ...
        const problems = result.error.errors
          ...
          .join('\n')
        throw new Error(`Env validation failed for extension "${ext.name}":\n${problems}`)
      }
      config = result.data
    }
    ...
    await ext.setup({
      ...
      dep(depExt: AnyExt) {
        if (!resolvedConfigs.has(depExt.name))
          throw new Error(`"${ext.name}" accessed dep "${depExt.name}" before it resolved`)
