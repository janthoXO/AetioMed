...
export const extension = defineExtension({
  ...
  envSchema: z
      ...
      if (isAllowLlms) {
        if (!data.ALLOWED_LLMS) {
          ctx.addIssue({
            ...
            message: "ALLOWED_LLMS is required when ALLOW_LLMS is enabled",
          });
        }
      } else {
        if (!data.LLM_PROVIDER || !data.LLM_MODEL) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "LLM_PROVIDER and LLM_MODEL are required",
          });
        }
      }
    })
    .transform((env) => {
      ...
      if (isAllowLlms && env.ALLOWED_LLMS) {
        allowedLlms = env.ALLOWED_LLMS.split(",").reduce(
          (acc, pair) => {
            const [provider, model] = pair.split(":");
            if (provider && model) {
              if (!acc[provider as "ollama" | "google"])
                acc[provider as "ollama" | "google"] = [];
  ...
  async setup({ config: parsedConfig, router, bus: extensionBus }) {
    ...
    console.log(
      `[core] Starting core extension with ${parsedConfig.isAllowLlms ? "dynamic LLMs" : parsedConfig.llmProvider + "/" + parsedConfig.llmModel}`
    );
  },
});
