...
// carries node output (a log record, in #141) and is gated by its own
// standard `OTEL_SDK_DISABLED`, never by a `FEATURES` flag — a deployer can
// run either channel independently of the other.
//
 ...
 * `initGraph`/`buildCaseGraph`. Always called, gated only by the standard
 * `OTEL_SDK_DISABLED` — with the SDK disabled this still returns a working
 * `NodeTracer`, just one backed by `@opentelemetry/api`'s own global no-op
 * tracer (no provider ever registered), rather than a bespoke no-op of
 * ours: one fewer thing to keep in sync with the real `Span` interface,
 * and exactly what "a no-op tracer otherwise" (issue 15 §5) means.
 *
